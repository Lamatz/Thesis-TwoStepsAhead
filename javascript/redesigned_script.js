...
function populateReportSummary() {
    ...
    // const periodText = periodValue !== '24' ? forecastSelect.options[forecastSelect.selectedIndex].text : "N/A";
    ...
    console.log("populating in this");
    // --- USE THIS CORRECTED LOGIC INSTEAD ---

    // Find the radio button that is currently checked
    const checkedRadioButton = document.querySelector('#forecast-period input[name="duration"]:checked');

    if (checkedRadioButton) {
        const periodValue = checkedRadioButton.value; // Will be "3", "6", "12", or "24"
        const periodText = checkedRadioButton.nextElementSibling.textContent; // "3 Hours", etc.

        document.getElementById("report-forecast-period").innerText = periodText;

        // Use optional chaining `?.` for safety when accessing properties
        const cumulativeRain = lastFetchedWeatherData.cumulative_rainfall?.[periodValue]?.toFixed(1) ?? "N/A";
        const intensityRain = lastFetchedWeatherData.rain_intensity?.[periodValue]?.toFixed(2) ?? "N/A";

        document.getElementById("report-cumulative-rain").innerText = cumulativeRain;
        document.getElementById("report-intensity-rain").innerText = intensityRain;

    } else {
        // This case handles if somehow no radio button is selected
        alert("Error: No forecast period was selected for the report.");
        document.getElementById("report-forecast-period").innerText = "N/A";
        document.getElementById("report-cumulative-rain").innerText = "N/A";
        document.getElementById("report-intensity-rain").innerText = "N/A";
    }
