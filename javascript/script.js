...
async function fetchWeatherData(latitude, longitude, date) {
    ...
    try {
        ...
        let response = await fetch(`http://127.0.0.1:5000/get_weather_final?latitude=${latitude}&longitude=${longitude}&date=${date}`);
        ...
        console.log("Full data object received from API:", data);
        ...
        document.getElementById("soil-moisture").value = data.soil_moisture !== undefined ? data.soil_moisture.toFixed(1) : "N/A - testing"; // Check for undefined
...
async function getSearchSuggestions(query) {
    ...
    try {
        ...
        data.suggestions.forEach(location => {
            ...
            try {
                let response = await fetch(`http://127.0.0.1:5000/get_weather?latitude=${latitude}&longitude=${longitude}`);
                if (!response.ok) {
                ...
                if (data.error) {
                    ...
                    rawSoilCode = null; // No raw code on error
                    soilLabel = "Error Fetching"; // Label for error
                    slopeValue = "Error"; // Slope for error
                    // --- END MODIFICATION ---
                    alert("Error fetching soil/slope data: " + data.error);
                } else {
                    // --- MODIFICATION: Get and store raw code, label, and slope value ---
                    rawSoilCode = data.soil_type; // Store the original SNUM string from backend
                    soilLabel = getSoilLabel(rawSoilCode); // Get the human-readable label
                    slopeValue = data.slope !== undefined && data.slope !== null ? data.slope : "Unknown"; // Get the slope value
                    // --- END MODIFICATION ---
                }
            } catch (error) {
            ...
            fetchedLocationData = {
            soil_type_snum: rawSoilCode,  // Store the original SNUM string
            soil_type_label: soilLabel,   // Store the human-readable label
            slope: slopeValue           // Store the raw slope value
        };
                    // --- END ADDITION ---
            ...
            `;
            // --- MODIFICATION: Update inputs with display values ---
            document.getElementById("slope").value = slopeValue; // Use slopeValue
            document.getElementById("soil-type").value = soilLabel; // Use soilLabel
            ...
            // --- MODIFICATION: Update marker popup with display values ---
...
map.on("click", async function (e) {
    ...
    try {
        let response = await fetch(`http://127.0.0.1:5000/get_weather?latitude=${lat}&longitude=${lng}`);
         if (!response.ok) {
             const errorText = await response.text();
             throw new Error(`HTTP error! status: ${response.status}, Details: ${errorText}`);
        }
